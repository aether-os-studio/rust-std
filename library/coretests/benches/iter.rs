...
fn bench_range_step_by_fold_u16(b: &mut Bencher) {
    ...
    b.iter(|| {
        let r = black_box(r.clone());
        r.step_by(64)
            .map(|x: u16| x ^ (x.wrapping_sub(1)))
            .fold(0u16, |acc, i| acc.wrapping_add(i))
    })
}
...
fn bench_partial_cmp(b: &mut Bencher) {
    b.iter(|| {
        (0..100000)
            .map(black_box)
            .partial_cmp((0..100000).map(black_box))
    })
}
