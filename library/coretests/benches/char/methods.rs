...
fn bench_to_ascii_uppercase(b: &mut Bencher) {
    b.iter(|| {
        CHARS
            ...
            .cycle()
            .take(10_000)
            .map(|c| black_box(c).to_ascii_uppercase())
            .min()
    })
}
...
fn bench_to_ascii_lowercase(b: &mut Bencher) {
    b.iter(|| {
        CHARS
            ...
            .cycle()
            .take(10_000)
            .map(|c| black_box(c).to_ascii_lowercase())
            .min()
    })
}
