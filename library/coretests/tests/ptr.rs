...
fn from_raw_parts() {
    ...
    assert_eq!(
        ptr::from_raw_parts_mut(address, vtable),
        trait_object.as_ptr()
    );
    assert_eq!(
        NonNull::from_raw_parts(NonNull::new(address).unwrap(), vtable),
        trait_object
    );
    ...
    assert_eq!(
        NonNull::from_raw_parts(NonNull::new(address).unwrap(), ()),
        array_ptr
    );
    ...
    assert_eq!(
        NonNull::from_raw_parts(NonNull::new(address).unwrap(), 5),
        slice_ptr
    );
}
