...
fn test_fmt_debug_of_raw_pointers() {
    ...
    fn check_fmt<T: Debug>(t: T, start: &str, contains: &str) {
        ...
        assert!(
            formatted.starts_with(start),
            "{formatted:?} doesn't start with {start:?}"
        );
        assert!(
            formatted.contains(contains),
            "{formatted:?} doesn't contain {contains:?}"
        );
    }

    assert_eq!(
        format!("{:?}", ptr::without_provenance_mut::<i32>(0x100)),
        "0x100"
    );
    assert_eq!(
        format!("{:?}", ptr::without_provenance::<i32>(0x100)),
        "0x100"
    );
...
fn test_estimated_capacity() {
    ...
    assert_eq!(
        format_args!("{}, hello!", { "World" }).estimated_capacity(),
        0
    );
    assert_eq!(
        ...
        for alternate in [true, false] {
            for sign_aware_zero_pad in [true, false] {
                for debug_as_hex in [None, Some(DebugAsHex::Lower), Some(DebugAsHex::Upper)] {
                    ...
                    assert_eq!(
                        formatting_options.get_sign_aware_zero_pad(),
                        sign_aware_zero_pad
                    );
