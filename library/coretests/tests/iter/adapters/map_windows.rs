...
mod drop_checks {
    ...
    impl DropInfo {
        const fn new() -> Self {
            Self {
                dropped_twice: AtomicBool::new(false),
        ...
        fn check(&self) {
            assert!(
                !self.dropped_twice.load(SeqCst),
...
fn output_n1() {
    ...
    assert_eq!(
        "x".chars().map_windows(|[c]| *c).collect::<Vec<_>>(),
        vec!['x']
    );
    assert_eq!(
        "abcd".chars().map_windows(|[c]| *c).collect::<Vec<_>>(),
        vec!['a', 'b', 'c', 'd']
    );
}
...
fn test_zero_sized_type() {
    ...
    struct Data;
    let data: Vec<_> = std::iter::repeat(Data)
        .take(10)
        .map_windows(|arr: &[Data; 5]| *arr)
        .collect();
