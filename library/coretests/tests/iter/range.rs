...
fn test_char_range() {
    ...
    assert!(
        (from..=to)
            .rev()
            .eq((from as u32..=to as u32).filter_map(char::from_u32).rev())
    );
...
fn test_ascii_char_range() {
    ...
    assert!(
        (from..=to)
            .rev()
            .eq((from as u8..=to as u8).filter_map(AsciiChar::from_u8).rev())
    );

    assert_eq!((AsciiChar::CapitalA..=AsciiChar::CapitalZ).count(), 26);
    assert_eq!(
        (AsciiChar::CapitalA..=AsciiChar::CapitalZ).size_hint(),
        (26, Some(26))
    );
    assert_eq!((AsciiChar::SmallA..=AsciiChar::SmallZ).count(), 26);
    assert_eq!(
        (AsciiChar::SmallA..=AsciiChar::SmallZ).size_hint(),
        (26, Some(26))
    );
    assert_eq!((AsciiChar::Digit0..=AsciiChar::Digit9).count(), 10);
    assert_eq!(
        (AsciiChar::Digit0..=AsciiChar::Digit9).size_hint(),
        (10, Some(10))
    );
}
...
fn test_range_advance_by() {
    ...
    assert_eq!(
        (r.start, r.end),
        (0u128 + usize::MAX as u128, u128::MAX - usize::MAX as u128)
    );
...
fn test_range_inclusive_size_hint() {
    ...
    assert_eq!(
        (isize::MIN..=isize::MAX - 1).size_hint(),
        (usize::MAX, Some(usize::MAX))
    );
