...
fn test_partial_cmp_by() {
    ...
    assert_eq!(
        xs().partial_cmp_by(xs().map(|x| x * x), f),
        Some(Ordering::Equal)
    );
    assert_eq!(
...
fn test_iterator_advance_by() {
    ...
    for i in 0..v.len() {
        ...
        assert_eq!(
            iter.advance_by(100),
            Err(NonZero::new(100 - (v.len() - 1 - i)).unwrap())
        );
    }

    assert_eq!(v.iter().advance_by(v.len()), Ok(()));
    assert_eq!(
        v.iter().advance_by(100),
        Err(NonZero::new(100 - v.len()).unwrap())
    );
}
...
fn test_iterator_size_hint() {
    ...
    assert_eq!(
        vi.clone().map_while(|_| None::<()>).size_hint(),
        (0, Some(10))
    );
...
fn test_try_reduce() {
    ...
    let max = v.into_iter().try_reduce(|x, y| {
        if x.parse::<usize>().ok()? > y.parse::<usize>().ok()? {
            Some(x)
        } else {
            Some(y)
        }
