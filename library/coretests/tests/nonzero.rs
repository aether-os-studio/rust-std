...
mod atom {
    ...
    pub const FOO_ATOM: Atom = Atom {
        index: unsafe { NonZero::new_unchecked(7) },
    };
}
...
fn nonzero_leading_zeros() {
    ...
    assert_eq!(
        NonZero::<usize>::new(1).unwrap().leading_zeros(),
        usize::BITS - 1
    );
    assert_eq!(
        NonZero::<isize>::new(1).unwrap().leading_zeros(),
        usize::BITS - 1
    );

    assert_eq!(NonZero::<u8>::new(u8::MAX >> 2).unwrap().leading_zeros(), 2);
    assert_eq!(
        NonZero::<i8>::new((u8::MAX >> 2) as i8)
            .unwrap()
            .leading_zeros(),
        2
    );
    assert_eq!(
        NonZero::<u16>::new(u16::MAX >> 2).unwrap().leading_zeros(),
        2
    );
    assert_eq!(
        NonZero::<i16>::new((u16::MAX >> 2) as i16)
            .unwrap()
            .leading_zeros(),
        2
    );
    assert_eq!(
        NonZero::<u32>::new(u32::MAX >> 2).unwrap().leading_zeros(),
        2
    );
    assert_eq!(
        NonZero::<i32>::new((u32::MAX >> 2) as i32)
            .unwrap()
            .leading_zeros(),
        2
    );
    assert_eq!(
        NonZero::<u64>::new(u64::MAX >> 2).unwrap().leading_zeros(),
        2
    );
    assert_eq!(
        NonZero::<i64>::new((u64::MAX >> 2) as i64)
            .unwrap()
            .leading_zeros(),
        2
    );
    assert_eq!(
        NonZero::<u128>::new(u128::MAX >> 2)
            .unwrap()
            .leading_zeros(),
        2
    );
    assert_eq!(
        NonZero::<i128>::new((u128::MAX >> 2) as i128)
            .unwrap()
            .leading_zeros(),
        2
    );
    assert_eq!(
        NonZero::<usize>::new(usize::MAX >> 2)
            .unwrap()
            .leading_zeros(),
        2
    );
    assert_eq!(
        NonZero::<isize>::new((usize::MAX >> 2) as isize)
            .unwrap()
            .leading_zeros(),
        2
    );
    ...
    assert_eq!(
        NonZero::<usize>::new(usize::MAX).unwrap().leading_zeros(),
        0
    );
...
fn nonzero_trailing_zeros() {
    ...
    assert_eq!(
        NonZero::<u128>::new(1 << 127).unwrap().trailing_zeros(),
        127
    );
    assert_eq!(
        NonZero::<i128>::new(1 << 127).unwrap().trailing_zeros(),
        127
    );

    assert_eq!(
        NonZero::<usize>::new(1 << (usize::BITS - 1))
            .unwrap()
            .trailing_zeros(),
        usize::BITS - 1
    );
    assert_eq!(
        NonZero::<isize>::new(1 << (usize::BITS - 1))
            .unwrap()
            .trailing_zeros(),
        usize::BITS - 1
    );
