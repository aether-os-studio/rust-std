...
fn copy_file_follows_dst_symlink() {
    ...
    assert!(
        check!(out_path_symlink.symlink_metadata())
            .file_type()
            .is_symlink()
    );
...
fn symlink_hard_link() {
    ...
    assert!(
        check!(fs::symlink_metadata(tmpdir.join("hard_link")))
            .file_type()
            .is_symlink()
    );
...
fn test_eq_windows_file_type() {
    ...
    assert_eq!(
        file1.metadata().unwrap().file_type(),
        file2.metadata().unwrap().file_type()
    );
...
fn test_rename_junction() {
    ...
    assert_eq!(
        fs::read_link(&dest).unwrap().file_name(),
        Some(not_exist.as_os_str())
    );
}
