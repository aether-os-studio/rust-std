...
impl FileAttr {
    pub fn modified(&self) -> io::Result<SystemTime> {
        SystemTime::new(
            self.stat.st_mtime.tv_sec as i64,
            self.stat.st_mtime.tv_nsec as i64,
        )
    }

    pub fn accessed(&self) -> io::Result<SystemTime> {
        SystemTime::new(
            self.stat.st_atime.tv_sec as i64,
            self.stat.st_atime.tv_nsec as i64,
        )
    }

    pub fn created(&self) -> io::Result<SystemTime> {
        SystemTime::new(
            self.stat.st_ctime.tv_sec as i64,
            self.stat.st_ctime.tv_nsec as i64,
        )
    }
}
...
impl DirBuilder {
    ...
    pub fn mkdir(&self, p: &Path) -> io::Result<()> {
        run_path_with_cstr(p, &|p| {
            cvt(unsafe { libc::mkdir(p.as_ptr(), self.mode) }).map(|_| ())
        })
    }
...
pub fn lchown(path: &Path, uid: u32, gid: u32) -> io::Result<()> {
    ...
    Err(io::const_error!(
        io::ErrorKind::Unsupported,
        "lchown not supported by vxworks"
    ))
}
...
pub fn chroot(dir: &Path) -> io::Result<()> {
    run_path_with_cstr(dir, &|dir| {
        cvt(unsafe { libc::chroot(dir.as_ptr()) }).map(|_| ())
    })
}
