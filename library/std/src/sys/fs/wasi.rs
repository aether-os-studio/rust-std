                ...
                match result {
                    Ok(read_bytes) => {
                        if read_bytes < buf.len() {
                            ...
                            self.state = ReadDirState::RunUntilExhaustion {
                                buf: mem::take(buf),
                                offset: 0,
                            };
                        } else {
                ...
                if contents.len() >= DIRENT_SIZE {
                    ...
                    if data.len() < dirent.d_namlen as usize {
                        ...
                        if let Some(next_read_offset) = *next_read_offset {
                            self.state = ReadDirState::FillBuffer {
                                next_read_offset,
                                buf: mem::take(buf),
                            };
                        } else {
...
pub fn osstr2str(f: &OsStr) -> io::Result<&str> {
    f.to_str()
        .ok_or_else(|| io::const_error!(io::ErrorKind::Uncategorized, "input must be utf-8"))
}
