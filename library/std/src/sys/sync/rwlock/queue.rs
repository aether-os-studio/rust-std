...
fn write_lock(state: State) -> Option<State> {
    if state.addr() & LOCKED == 0 {
        Some(state.map_addr(|addr| addr | LOCKED))
    } else {
        None
    }
}
...
fn read_lock(state: State) -> Option<State> {
    if state.addr() & QUEUED == 0 && state.addr() != LOCKED {
        Some(without_provenance_mut(
            state.addr().checked_add(SINGLE)? | LOCKED,
        ))
    } else {
        None
    }
...
impl RwLock {
    ...
    fn lock_contended(&self, write: bool) {
        ...
        loop {
            ...
            if let Err(new) = self
                .state
                .compare_exchange_weak(state, next, AcqRel, Relaxed)
            {
    ...
    pub unsafe fn read_unlock(&self) {
        match self.state.fetch_update(Release, Acquire, |state| {
            if state.addr() & QUEUED == 0 {
                ...
                Some(if count > 0 {
                    without_provenance_mut(count | LOCKED)
                } else {
                    UNLOCKED
                })
    ...
    pub unsafe fn write_unlock(&self) {
        if let Err(state) =
            self.state
                .compare_exchange(without_provenance_mut(LOCKED), UNLOCKED, Release, Relaxed)
        {
    ...
    unsafe fn unlock_contended(&self, state: State) {
        debug_assert_eq!(
            state.addr() & (DOWNGRADED | QUEUED | LOCKED),
            QUEUED | LOCKED
        );
        ...
        loop {
            ...
            match self
                .state
                .compare_exchange_weak(current, next, AcqRel, Relaxed)
            {
    ...
    unsafe fn downgrade_slow(&self, mut state: State) {
        ...
        loop {
            if state.addr() & QUEUE_LOCKED != 0 {
                ...
                match self
                    .state
                    .compare_exchange_weak(state, next, Release, Relaxed)
                {
                    Ok(_) => return,
                    Err(new) => state = new,
                }
    ...
    unsafe fn unlock_queue(&self, mut state: State) {
        debug_assert_eq!(
            state.addr() & (QUEUED | QUEUE_LOCKED),
            QUEUED | QUEUE_LOCKED
        );
        ...
        loop {
                ...
                && let Some(prev) = unsafe { tail.as_ref().prev.get() }
            {
                ...
                if let Err(new) = self
                    .state
                    .compare_exchange_weak(state, next, Release, Acquire)
                {
                ...
                // Clear the queue.
                let next = if downgrade {
                    ptr::without_provenance_mut(SINGLE | LOCKED)
                } else {
                    UNLOCKED
                };
                if let Err(new) = self
