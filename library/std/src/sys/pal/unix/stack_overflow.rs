...
mod imp {
    ...
    pub unsafe fn make_handler(main_thread: bool) -> Handler {
        ...
        if stack.ss_flags & SS_DISABLE != 0 {
            ...
            Handler {
                data: stack.ss_sp as *mut libc::c_void,
            }
        } else {
            Handler::null()
        }
    ...
    unsafe fn get_stack_start() -> Option<*mut libc::c_void> {
        let mut current_stack: libc::stack_t = crate::mem::zeroed();
        assert_eq!(
            libc::pthread_stackseg_np(libc::pthread_self(), &mut current_stack),
            0
        );
    ...
    unsafe fn current_guard() -> Option<Range<usize>> {
        ...
        if e == 0 {
            let mut guardsize = 0;
            assert_eq!(
                libc::pthread_attr_getguardsize(attr.as_ptr(), &mut guardsize),
                0
            );
            if guardsize == 0 {
            ...
            let mut size = 0;
            assert_eq!(
                libc::pthread_attr_getstack(attr.as_ptr(), &mut stackptr, &mut size),
                0
            );
