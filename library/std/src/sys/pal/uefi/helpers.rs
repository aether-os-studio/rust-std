...
pub(crate) fn image_handle_protocol<T>(protocol_guid: Guid) -> io::Result<NonNull<T>> {
    let system_handle = uefi::env::try_image_handle().ok_or(io::const_error!(
...
pub(crate) fn device_path_to_text(path: NonNull<device_path::Protocol>) -> io::Result<OsString> {
    ...
    Err(io::const_error!(
        io::ErrorKind::NotFound,
        "no device path to text protocol found"
    ))
}
...
impl<T> OwnedProtocol<T> {
    ...
    pub(crate) unsafe fn create(protocol: T, mut guid: r_efi::efi::Guid) -> io::Result<Self> {
        ...
        let handle = NonNull::new(handle).ok_or(io::const_error!(
...
pub(crate) fn os_string_from_raw(ptr: *mut r_efi::efi::Char16) -> Option<OsString> {
    ...
    Some(OsString::from_wide(unsafe {
        slice::from_raw_parts(ptr.cast(), path_len)
    }))
}
...
pub(crate) fn os_string_to_raw(s: &OsStr) -> Option<Box<[r_efi::efi::Char16]>> {
    let temp = s
        .encode_wide()
        .chain(Some(0))
        .collect::<Box<[r_efi::efi::Char16]>>();
    if temp[..temp.len() - 1].contains(&0) {
        None
    } else {
        Some(temp)
    }
...
pub(crate) fn get_device_path_from_map(map: &Path) -> io::Result<BorrowedDevicePath<'static>> {
    let shell = open_shell().ok_or(io::const_error!(
        io::ErrorKind::NotFound,
        "UEFI Shell not found"
    ))?;
    let mut path = os_string_to_raw(map.as_os_str()).ok_or(io::const_error!(
    ...
    let protocol = NonNull::new(protocol).ok_or(io::const_error!(
        io::ErrorKind::NotFound,
        "UEFI Shell mapping not found"
    ))?;
...
impl ServiceProtocol {
    ...
    pub(crate) fn open(service_guid: r_efi::efi::Guid) -> io::Result<Self> {
        ...
        Err(io::const_error!(
            io::ErrorKind::NotFound,
            "no service binding protocol found"
        ))
    }
