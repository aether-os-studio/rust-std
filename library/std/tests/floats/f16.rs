...
macro_rules! assert_f16_biteq {
    ($a:expr, $b:expr) => {
        ...
        assert_eq!(
            lb, rb,
            "float {l:?} ({lb:#04x}) is not bitequal to {r:?} ({rb:#04x})"
        );
    };
}
...
fn test_asinh() {
    ...
    assert_approx_eq!(
        2.0f16.asinh(),
        1.443635475178810342493276740273105f16,
        TOL_0
    );
    assert_approx_eq!(
        (-2.0f16).asinh(),
...
fn test_acosh() {
    ...
    assert_approx_eq!(
        2.0f16.acosh(),
        1.31695789692481670862504634730796844f16,
        TOL_0
    );
    assert_approx_eq!(
        3.0f16.acosh(),
        1.76274717403908605046521864995958461f16,
        TOL_0
    );
...
fn test_atanh() {
    ...
    assert_approx_eq!(
        0.5f16.atanh(),
        0.54930614433405484569762261846126285f16,
        TOL_0
    );
    assert_approx_eq!(
        (-0.5f16).atanh(),
...
fn test_total_cmp() {
    ...
    assert_eq!(
        Ordering::Equal,
        (-f16::MIN_POSITIVE).total_cmp(&-f16::MIN_POSITIVE)
    );
    ...
    assert_eq!(
        Ordering::Less,
        (-f16::MIN_POSITIVE).total_cmp(&-max_subnorm())
    );
    ...
    assert_eq!(
        Ordering::Greater,
        (-max_subnorm()).total_cmp(&-f16::MIN_POSITIVE)
    );
    assert_eq!(
        Ordering::Greater,
