...
macro_rules! assert_f32_biteq {
    ($left : expr, $right : expr) => {
        ...
        assert_eq!(
            lb, rb,
            "float {l} ({lb:#010x}) is not bitequal to {r} ({rb:#010x})"
        );
    };
}
...
fn test_asinh() {
    ...
    assert_approx_eq!(
        (-3000.0f32).asinh(),
        -8.699514775987968673236893537700647f32
    );
...
fn test_total_cmp() {
    ...
    assert_eq!(
        Ordering::Equal,
        (-f32::MIN_POSITIVE).total_cmp(&-f32::MIN_POSITIVE)
    );
    ...
    assert_eq!(
        Ordering::Less,
        (-f32::MIN_POSITIVE).total_cmp(&-max_subnorm())
    );
    ...
    assert_eq!(
        Ordering::Greater,
        (-max_subnorm()).total_cmp(&-f32::MIN_POSITIVE)
    );
    assert_eq!(
        Ordering::Greater,
