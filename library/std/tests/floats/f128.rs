...
fn test_powi() {
    ...
    assert_approx_eq!(
        (-3.1f128).powi(2),
        9.6100000000000005506706202140776519387,
        TOL
    );
    assert_approx_eq!(
        5.9f128.powi(-2),
        0.028727377190462507313100483690639638451,
        TOL
    );
...
fn test_powf() {
    ...
    assert_approx_eq!(
        3.4f128.powf(4.5),
        246.40818323761892815995637964326426756,
        TOL_IMPR
    );
    assert_approx_eq!(
        2.7f128.powf(-3.2),
        0.041652009108526178281070304373500889273,
        TOL_IMPR
    );
    assert_approx_eq!(
        (-3.1f128).powf(2.0),
...
fn test_exp() {
    ...
    assert_approx_eq!(
        148.41315910257660342111558004055227962348775,
        5.0f128.exp(),
        TOL
    );
...
fn test_log() {
    ...
    assert_approx_eq!(
        2.3f128.log(3.5),
        0.66485771361478710036766645911922010272,
        TOL
    );
...
fn test_log2() {
    ...
    assert_approx_eq!(
        1.0f128.exp().log2(),
        1.4426950408889634073599246810018921381,
        TOL
    );
...
fn test_log10() {
    ...
    assert_approx_eq!(
        2.3f128.log10(),
        0.36172783601759284532595218865859309898,
        TOL
    );
    assert_approx_eq!(
        1.0f128.exp().log10(),
        0.43429448190325182765112891891660508222,
        TOL
    );
...
fn test_to_degrees() {
    ...
    assert_approx_eq!(
        (-5.8f128).to_degrees(),
        -332.31552117587745090765431723855668471,
        TOL
    );
...
fn test_to_radians() {
    ...
    assert_approx_eq!(
        154.6f128.to_radians(),
        2.6982790235832334267135442069489767804,
        TOL
    );
    assert_approx_eq!(
        (-332.31f128).to_radians(),
        -5.7999036373023566567593094812182763013,
        TOL
    );
...
fn test_asinh() {
    ...
    assert_approx_eq!(
        2.0f128.asinh(),
        1.443635475178810342493276740273105f128,
        TOL_IMPR
    );
    assert_approx_eq!(
        (-2.0f128).asinh(),
...
fn test_acosh() {
    ...
    assert_approx_eq!(
        2.0f128.acosh(),
        1.31695789692481670862504634730796844f128,
        TOL_IMPR
    );
    assert_approx_eq!(
        3.0f128.acosh(),
        1.76274717403908605046521864995958461f128,
        TOL_IMPR
    );
...
fn test_atanh() {
    ...
    assert_approx_eq!(
        0.5f128.atanh(),
        0.54930614433405484569762261846126285f128,
        TOL_IMPR
    );
    assert_approx_eq!(
        (-0.5f128).atanh(),
...
fn test_float_bits_conv() {
    ...
    assert_approx_eq!(
        f128::from_bits(0x40029000000000000000000000000000),
        12.5,
        TOL_PRECISE
    );
    assert_approx_eq!(
        f128::from_bits(0x40094e40000000000000000000000000),
        1337.0,
        TOL_PRECISE
    );
    assert_approx_eq!(
        f128::from_bits(0xc002c800000000000000000000000000),
        -14.25,
        TOL_PRECISE
    );
...
fn test_total_cmp() {
    ...
    assert_eq!(
        Ordering::Equal,
        (-f128::INFINITY).total_cmp(&-f128::INFINITY)
    );
    ...
    assert_eq!(
        Ordering::Equal,
        (-f128::MIN_POSITIVE).total_cmp(&-f128::MIN_POSITIVE)
    );
