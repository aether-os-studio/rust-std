...
fn test_pathbuf_leak() {
    ...
    unsafe {
        drop(String::from_raw_parts(
            leaked.as_mut_os_str() as *mut OsStr as _,
            len,
            cap,
        ))
    }
...
fn test_unix_absolute() {
    ...
    assert_eq!(
        absolute(relative).unwrap().as_os_str(),
        expected.as_os_str()
    );
    ...
    assert_eq!(
        absolute("./a").unwrap().as_os_str(),
        curdir.join("a").as_os_str()
    );
    assert_eq!(
        absolute("../a").unwrap().as_os_str(),
        curdir.join("../a").as_os_str()
...
fn test_windows_absolute() {
    ...
    assert_eq!(
        absolute(r"COM1").unwrap().as_os_str(),
        Path::new(r"\\.\COM1").as_os_str()
    );
}
