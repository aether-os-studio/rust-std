...
fn test_should_panic_but_succeeds() {
    let should_panic_variants = [
        ShouldPanic::Yes,
        ShouldPanic::YesWithMessage("error message"),
    ];
...
fn test_time_options_threshold() {
    ...
    let test_vector = [
        ...
        (
            TestType::IntegrationTest,
            integration.warn.as_millis() - 1,
            false,
            false,
        ),
        (
            TestType::IntegrationTest,
            integration.warn.as_millis(),
            true,
            false,
        ),
        (
            TestType::IntegrationTest,
            integration.critical.as_millis(),
            true,
            true,
        ),
    ...
    for (test_type, time, expected_warn, expected_critical) in test_vector.iter() {
        ...
        assert_eq!(
            options.is_critical(&test_desc, &exec_time),
            *expected_critical
        );
    }
}
...
fn shuffle_tests() {
    ...
    let left = sample_tests()
        .into_iter()
        .enumerate()
        .map(|(i, e)| (TestId(i), e))
        .collect::<Vec<_>>();
    let mut right = sample_tests()
        .into_iter()
        .enumerate()
        .map(|(i, e)| (TestId(i), e))
        .collect::<Vec<_>>();
    ...
    assert!(
        left.iter()
            .zip(&right)
            .all(|(a, b)| a.1.desc.name == b.1.desc.name)
    );
    ...
    assert!(
        left.iter()
            .zip(right)
            .any(|(a, b)| a.1.desc.name != b.1.desc.name)
    );
}
...
fn order_depends_on_more_than_seed() {
    ...
    let mut left = left_tests
        .into_iter()
        .enumerate()
        .map(|(i, e)| (TestId(i), e))
        .collect::<Vec<_>>();
    let mut right = right_tests
        .into_iter()
        .enumerate()
        .map(|(i, e)| (TestId(i), e))
        .collect::<Vec<_>>();
