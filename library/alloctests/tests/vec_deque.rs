...
fn test_try_fold_empty() {
    assert_eq!(
        Some(0),
        VecDeque::<u32>::new().iter().try_fold(0, |_, _| None)
    );
}
...
fn test_try_fold_none() {
    ...
    assert_eq!(
        None,
        v.into_iter()
            .try_fold(0, |a, b| if b < 11 { Some(a + b) } else { None })
    );
}
...
fn test_collect_from_into_iter_keeps_allocation() {
    ...
    fn check(buf: *const i32, last: *const i32, mut it: impl Iterator<Item = i32>) {
        ...
        assert_eq!(
            v.as_slices(),
            ([7, 2, 3, 4, 5, 6].as_slice(), [].as_slice())
        );
        v.push_front(8);
        assert_eq!(
            v.as_slices(),
            ([8, 7, 2, 3, 4, 5, 6].as_slice(), [].as_slice())
        );
        ...
        assert_eq!(
            v.as_slices(),
            ([9].as_slice(), [8, 7, 2, 3, 4, 5, 6].as_slice())
        );
        assert_eq!(v.capacity(), 13);
    }
}
