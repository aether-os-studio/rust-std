...
fn test_format_macro_interface() {
    ...
    t!(
        format!("{:?}", "foo\0bar\x01baz\u{7f}q\u{75}x"),
        r#""foo\0bar\u{1}baz\u{7f}qux""#
    );
    ...
    if cfg!(target_pointer_width = "32") {
        t!(
            format!("{:#p}", ptr::without_provenance::<isize>(0x1234)),
            "0x00001234"
        );
        t!(
            format!("{:#p}", ptr::without_provenance_mut::<isize>(0x1234)),
            "0x00001234"
        );
    } else {
        t!(
    ...
    t!(
        format!("{foo} {1} {bar} {0}", 0, 1, foo = 2, bar = 3),
        "2 1 3 0"
    );
    t!(format!("{} {0}", "a"), "a a");
    ...
    t!(
        format!("{:x}{0:X}{a:x}{:X}{1:x}{a:X}", 13, 14, a = 15),
        "dDfEeF"
    );
    ...
    // Test that pointers don't get truncated.
    {
        ...
        t!(
            format!("{:p}", std::ptr::without_provenance::<isize>(val)),
            exp
        );
    }
