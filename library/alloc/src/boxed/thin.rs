...
impl<T> ThinBox<T> {
    ...
    pub fn try_new(value: T) -> Result<Self, core::alloc::AllocError> {
        let meta = ptr::metadata(&value);
        WithOpaqueHeader::try_new(meta, value).map(|ptr| ThinBox {
            ptr,
            _marker: PhantomData,
        })
    }
        ...
        T: Unsize<Dyn>,
    {
        ...
        let alloc: &<Dyn as Pointee>::Metadata = const {
            ...
            unsafe {
                ...
                let metadata_offset = alloc_size
                    .checked_sub(size_of::<<Dyn as Pointee>::Metadata>())
                    .unwrap();
        ...
        let value_ptr = unsafe { (alloc as *const <Dyn as Pointee>::Metadata).add(1) }
            .cast::<T>()
            .cast_mut();
